...
keys = [
    ...
    Key([], "XF86AudioRaiseVolume", lazy.spawn("pactl set-sink-volume @DEFAULT_SINK@ +5%"), desc="decrease volume"),
    Key([], "XF86AudioLowerVolume", lazy.spawn("pactl set-sink-volume @DEFAULT_SINK@ -5%"), desc="increase volume"),
...
azerty_keys = ["ampersand", "eacute", "quotedbl", "apostrophe", "parenleft", "minus", "egrave", "underscore", "ccedilla"]

for group, key in zip(groups, azerty_keys):
    keys.extend(
        [
            # mod + AZERTY key = switch to group
            Key([mod], key, lazy.group[group.name].toscreen(),
                desc=f"Switch to group {group.name}"),
                
            # mod + shift + AZERTY key = move focused window to group
